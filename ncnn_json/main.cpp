...
int main()
{
    std::vector<std::vector<int>> x = get_inputs("data.json");
    std::vector<int> labels = get_labels("data.json");
    ...
    // 一次推理
    ncnn::Extractor extractor = net.create_extractor();     // 初始化ncnn推理器
    ncnn::Mat input = array2ncnnmat(x[2]);     // 把vector<int>转换成ncnn的mat
    extractor.input("in0", input);
    ncnn::Mat output;
    extractor.extract("out0", output);

    output_json("result.json", "test", get_max_index(output));
